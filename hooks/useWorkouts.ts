...
export function useWorkouts() {
  ...
  const getWorkouts = useCallback(async (userId: string, startDate?: string, endDate?: string) => {
    ...
    try {
      let query = supabaseClient.from("workouts").select("*").eq("user_id", userId).order("date", { ascending: false })
  ...
  const createWorkout = useCallback(async (workout: WorkoutInsert) => {
    ...
    try {
      const { data, error } = await supabaseClient.from("workouts").insert(workout).select().single()
  ...
  const updateWorkout = useCallback(async (id: string, workout: WorkoutUpdate) => {
    ...
    try {
      const { data, error } = await supabaseClient.from("workouts").update(workout).eq("id", id).select().single()
