...
export function useTemplates() {
  ...
  const getTemplate = useCallback(async (id: string) => {
    ...
    try {
      const { data, error } = await supabaseClient.from("workout_templates").select("*").eq("id", id).single()
  ...
  const createTemplate = useCallback(async (template: WorkoutTemplateInsert) => {
    ...
    try {
      const { data, error } = await supabaseClient.from("workout_templates").insert(template).select().single()
  ...
  const deleteTemplate = useCallback(async (id: string) => {
    ...
    try {
      const { error } = await supabaseClient.from("workout_templates").delete().eq("id", id)
