...
export function TemplateWorkoutForm({ templateId, onSuccess }: TemplateWorkoutFormProps) {
  const { user } = useAuth()
  const { createWorkout, loading: workoutLoading, error: workoutError } = useWorkouts()
  const { getTemplate, loading: templateLoading, error: templateError } = useTemplates()
  const router = useRouter()

  const [date, setDate] = useState(formatDateForInput(new Date()))
  const [exercises, setExercises] = useState<(TemplateExercise & { selected: boolean })[]>([])
        ...
        <CardContent className="space-y-4">
          {(error || workoutError || templateError) && (
            <Alert variant="destructive">
              <AlertDescription>{error || workoutError || templateError}</AlertDescription>
            </Alert>
          )}
          ...
          <div className="space-y-4">
            ...
            {exercises.length === 0 ? (
              <div className="rounded-md border border-dashed p-6 text-center">
                <p className="text-sm text-muted-foreground">No exercises found in this template.</p>
              </div>
            ) : (
              <div className="space-y-4">
